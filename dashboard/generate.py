...
<style>
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .posts-list.collapsed {{ max-height: 160px; overflow: hidden; }}
  .posts-list.collapsed::after {{
  ...
  .expand-btn {{ display: block; margin-top: 6px; font-size: 0.75rem; color: #0284c7;
...
function expandPosts() {{
  document.getElementById("posts-list").classList.remove("collapsed");
...
function showDetail(d) {{
  ...
  let posts;
  if (!d.sample_posts.length) {{
    posts = "<span style='color:#94a3b8'>No posts</span>";
  }} else {{
    const items = d.sample_posts.map(p => `
    ...
    const expandBtn = d.sample_posts.length > 4
      ? `<button class="expand-btn" id="expand-btn" onclick="expandPosts()">See all ${{d.sample_posts.length}} threads ↓</button>`
      : "";
    posts = `<div class="posts-list${{d.sample_posts.length > 4 ? " collapsed" : ""}}" id="posts-list">${{items}}</div>${{expandBtn}}`;
  }}
